...
Following @Fletcher2015, fishing sustainability risk was defined as negligible, low, medium, high or severe. These risk levels were calculated as the product of consequence and likelihood (see definition below). The likelihood of each consequence level was calculated for each line of detailed below. 

For spatio-temporal changes in catch and effort, likelihood values were derived from spatial and temporal dynamics of the total number of blocks fished within each species' core area, the proportion of blocks with catch as well as market dynamics and legislated species protection (Appendix 6). 

For the data-limited COMs analyses, likelihood values were calculated based on probabilities of the total catch (for the last five years) of a species being above, between or below catch reference points. Rather than using a point estimate of total catch, a normal distribution was used with mean and standard deviation set at the mean and standard deviation of the observed total catch. For each species, the limit reference point for catch was set to the median value for MSY produced by the COMs models, i.e. the limit reference point is considered to be breached if there is greater than 50% probability of MSY being exceeded. The threshold reference point was set to 0.8 of the MSY median value produced by the COMs models, i.e. the threshold is considered to be breached if there is greater than 20% probability (and thus still appreciable risk) of MSY being exceeded. These limit and threshold catch reference points were derived based on the theoretical expectation that, for any stock, catches above MSY, over any period other than the short term, will result in overfishing and should thus be avoided (@Martell2012). The target reference point was set to 0.5 of the MSY median value produced by the COMs models, i.e. to a level of catch that is much lower than the median expectation for MSY and, taking into account full uncertainty for MSY from these data-limited approaches, was able to be shown for all species in this assessment to have zero chance of exceeding MSY.
...
for(i in 1:length(plots))
{
  ...
  cat(sprintf('![%s](%s)', paste('Figure A.3.1.',i,'. ',Titl,'. Spatial distribution of nominal catch per unit of effort (kg per km gillnet hour) for the TDGDLF. Also shown are the annual sea surface temperature anomalies.',sep=''), 
               filename), sep = '\n')
  cat("\n")
}
```
...
For each assessed species, a single Consequence and Likelihood value was selected based on the spatial and temporal dynamics of effort, the total number of blocks fished and the proportion of blocks fished with catch within each species core area. Market dynamics and species protection were also taken in consideration. For angel sharks, most of the catch has been taken by the Great Australian Bight (GAB) Trawl Fishery (managed by the Commonwealth) whereas for copper shark a large proportion of the catch has been taken by South Australia's Marine Scale Fishery (SA MSF). No spatio-temporal catch distribution data were available from these fisheries. However, effort has substantially decreased in recent years due to the introduction of a range of management measures (@afmaHarvestStrategyFramework2009, @steerAssessmentSouthAustralian2020, @nitschkeSpatiotemporalVariabilityDemersal2022). 
